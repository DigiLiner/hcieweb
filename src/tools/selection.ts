...
import { drawShapesToCtx } from '../canvas/layers';

declare global {
  interface Window {
    
    // Explicitly re-expose needed selection tools 
    buildRectSelection: typeof buildRectSelection;
    buildEllipseSelection: typeof buildEllipseSelection;
    buildLassoSelection: typeof buildLassoSelection;
    buildPolygonalSelectionRef: typeof buildPolygonalSelection; 
    buildSelectionFromMask: typeof buildSelectionFromMask;
    drawSelectionBorder: typeof drawSelectionBorder;
    getRectBorder: typeof getRectBorder;
    getEllipseBorder: typeof getEllipseBorder;
    selectAll: typeof selectAll;
    deselect: typeof deselect;
    invertSelection: typeof invertSelection;
    isPointInSelection: typeof isPointInSelection;
    getMaskBounds: typeof getMaskBounds;
    moveSelectionMask: typeof moveSelectionMask;
    moveSelectionContent: typeof moveSelectionContent;
    finalizeMoveContent: typeof finalizeMoveContent;
    magicWandSelection: typeof magicWandSelection;
  }
}
 ...
 */
     ...
     */
    function fillShapeMask(ctx: CanvasRenderingContext2D, shape: string, x1: number, y1: number, x2: number, y2: number, lassoPoints?: Array<{x:number,y:number}>) {
     ...
     */
export function getRectBorder(x1: number, y1: number, x2: number, y2: number) {
        const left = Math.min(x1, x2);
        const top = Math.min(y1, y2);
        const right = Math.max(x1, x2);
        const bottom = Math.max(y1, y2);
...
export function getEllipseBorder(x1: number, y1: number, x2: number, y2: number, steps?: number) {
        steps = steps || 64;
        const cx = (x1 + x2) / 2;
        const cy = (y1 + y2) / 2;
        const rx = Math.abs(x2 - x1) / 2;
     ...
     */
    function combineMask(newMaskCanvas: HTMLCanvasElement, mode: import('../core/types').SelectionMode) {
        ...
        const resultCanvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const rctx = resultCanvas.getContext("2d");
        if (!rctx) return newMaskCanvas;

        // Start with existing
        rctx.drawImage(g.selectionCanvas as HTMLCanvasElement, 0, 0);
        ...
        if (mode === "add") {
            ...
            const newCtx = newMaskCanvas.getContext("2d");
            if (newCtx) {
     ...
     */
export function getMaskBounds(maskCanvas: import('../core/types').CanvasLike) {
        const w = maskCanvas.width;
        const h = maskCanvas.height;
        const ctx = (maskCanvas as HTMLCanvasElement).getContext("2d", { willReadFrequently: true }) || (maskCanvas as OffscreenCanvas).getContext("2d");
        if (!ctx) return null;
     ...
     */
    function extractSelectionBorder(maskCanvas: import('../core/types').CanvasLike): import('../core/types').Point[][] {
        ...
        const paths: import('../core/types').Point[][] = [];
     ...
     */
    function traceContour(startX: number, startY: number, w: number, h: number, isSolid: (x: number, y: number) => boolean, visited: Uint8Array): import('../core/types').Point[] {
        visited[startY * w + startX] = 1;
        const path: import('../core/types').Point[] = [{ x: startX, y: startY }];
        let currX = startX;
        let currY = startY;
     ...
     */
export function buildRectSelection(x1: number, y1: number, x2: number, y2: number, mode: import('../core/types').SelectionMode) {
        const w = width();
        const h = height();
        if (w <= 0 || h <= 0) return;
        const canvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const ctx = canvas.getContext("2d");
        if (ctx) fillShapeMask(ctx, "rect", x1, y1, x2, y2);
        g.selectionCanvas = combineMask(canvas, mode);
        g.selectionBorder = extractSelectionBorder(g.selectionCanvas as HTMLCanvasElement);
     ...
     */
export function buildEllipseSelection(x1: number, y1: number, x2: number, y2: number, mode: import('../core/types').SelectionMode) {
        const w = width();
        const h = height();
        if (w <= 0 || h <= 0) return;
        const canvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const ctx = canvas.getContext("2d");
        if (ctx) fillShapeMask(ctx, "ellipse", x1, y1, x2, y2);
        g.selectionCanvas = combineMask(canvas, mode);
        g.selectionBorder = extractSelectionBorder(g.selectionCanvas as HTMLCanvasElement);
        g.isSelectionActive = true;
    }
     ...
     */
export function buildLassoSelection(points: Array<{x: number, y: number}>, mode: import('../core/types').SelectionMode) {
        const w = width();
        const h = height();
        if (w <= 0 || h <= 0 || !points || points.length < 2) return;
        const canvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const ctx = canvas.getContext("2d");
        if (ctx) fillShapeMask(ctx, "lasso", 0, 0, 0, 0, points);
        g.selectionCanvas = combineMask(canvas, mode);
        g.selectionBorder = extractSelectionBorder(g.selectionCanvas as HTMLCanvasElement);
        g.isSelectionActive = true;
    }
     ...
     */
export function buildPolygonalSelection(points: Array<{x: number, y: number}>, mode: import('../core/types').SelectionMode) {
        const w = width();
        const h = height();
        if (w <= 0 || h <= 0 || !points || points.length < 2) return;
        const canvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const ctx = canvas.getContext("2d");
        if (ctx) fillShapeMask(ctx, "poly", 0, 0, 0, 0, points);
        g.selectionCanvas = combineMask(canvas, mode);
        g.selectionBorder = extractSelectionBorder(g.selectionCanvas as HTMLCanvasElement);
        g.isSelectionActive = true;
    }
...
export function drawSelectionBorder(ctx: CanvasRenderingContext2D, offsetX = 0, offsetY = 0) {
...
export function deselect() {
        if (g.isTransforming && typeof (window as any).transformUI !== 'undefined') {
            (window as any).transformUI.commit();
        }
...
export function invertSelection() {
        ...
        const ctx = (g.selectionCanvas as HTMLCanvasElement).getContext("2d");
        if (!ctx) return;
...
export function isPointInSelection(x: number, y: number) {
        ...
        const ctx = (g.selectionCanvas as HTMLCanvasElement).getContext("2d", { willReadFrequently: true });
        if (!ctx) return false;
     ...
     */
export function buildSelectionFromMask(maskCanvas: HTMLCanvasElement, mode: import('../core/types').SelectionMode) {
        if (!maskCanvas) return;
     ...
     * Move the existing selection mask.
     */
export function moveSelectionMask(dx: number, dy: number) {
        if (!g.isSelectionActive || !g.selectionCanvas) return;
        const w = g.selectionCanvas.width;
        const h = g.selectionCanvas.height;
        const newCanvas = g.createCanvas(w, h) as HTMLCanvasElement;
        const nctx = newCanvas.getContext("2d");
        if (nctx) {
            nctx.drawImage(g.selectionCanvas as HTMLCanvasElement, dx, dy);
     ...
     */
    export class SelectionTransformUI {
        ...
        public originalShapes: any[] | null = null;
        public originalLayerIndex: number = -1;
        ...
        show(bounds: { x: number, y: number, w: number, h: number }) {
            this.active = true;
        ...
        updatePosition(bounds: { x: number, y: number, w: number, h: number }) {
        ...
        cancel() {
            ...
            if (floatingContent) {
                ...
                if (layer && this.originalImageData) {
                    (layer.ctx as CanvasRenderingContext2D).putImageData(this.originalImageData, 0, 0);
...
export let floatingContent: any = null;
 // { canvas, x, y, originalLayerIndex }
export function moveSelectionContent(dx: number, dy: number) {
        if (!g.isSelectionActive || !g.selectionCanvas) return;
        ...
        if (!floatingContent) {
            ...
            transformUI.originalImageData = (activeLayer.ctx as CanvasRenderingContext2D).getImageData(0, 0, activeLayer.canvas.width, activeLayer.canvas.height);
            ...
            if (activeLayer.shapes && activeLayer.shapes.length > 0) {
                ...
                if (typeof drawShapesToCtx === 'function') {
                    drawShapesToCtx(activeLayer.ctx as CanvasRenderingContext2D, activeLayer.shapes);
                    // Clear shapes from vector array (they are now pixels)
                    activeLayer.shapes = [];
                }
            }

            const bounds = getMaskBounds(g.selectionCanvas as HTMLCanvasElement);
            if (!bounds) return;

            // 1. Create floating canvas
            const fCanvas = g.createCanvas(bounds.w, bounds.h) as HTMLCanvasElement;
            const fctx = fCanvas.getContext("2d");

            // Mask the source layer pixels
            const tempCanvas = g.createCanvas(g.image_width, g.image_height) as HTMLCanvasElement;
            const tctx = tempCanvas.getContext("2d");
            if (tctx && fctx) {
                tctx.drawImage(activeLayer.canvas as HTMLCanvasElement, 0, 0);
                tctx.globalCompositeOperation = "destination-in";
                tctx.drawImage(g.selectionCanvas as HTMLCanvasElement, 0, 0);

                // Draw into floating canvas
                fctx.drawImage(tempCanvas, -bounds.x, -bounds.y);

                // 2. Erase from original layer
                const lctx = activeLayer.ctx as CanvasRenderingContext2D;
                lctx.save();
                lctx.globalCompositeOperation = "destination-out";
                lctx.drawImage(g.selectionCanvas as HTMLCanvasElement, 0, 0);
                lctx.restore();
...
export function finalizeMoveContent() {
        if (!floatingContent) return;
        const activeLayer = layers[g.activeLayerIndex];
        if (activeLayer) {
            (activeLayer.ctx as CanvasRenderingContext2D).drawImage(floatingContent.canvas, floatingContent.x, floatingContent.y);
        }
        floatingContent = null;
        g.floatingContent = null;
        if (window.renderLayers) window.renderLayers();
     ...
     */
export function magicWandSelection(startX: number, startY: number, sourceCtx: CanvasRenderingContext2D, tolerance: number, mode: import('../core/types').SelectionMode) {
        ...
        console.log(`[DEBUG] magicWandSelection: Flood fill complete. Found ${pixelCount} pixels.`);

        // 2. Create mask canvas from maskData
        const mCanvas = g.createCanvas(w, h) as HTMLCanvasElement;
        ...
        if (mctx) {
            ...
            buildSelectionFromMask(mCanvas, mode);
        }
    }

    window.addEventListener('toolChanged', (e: Event) => {
        const detail = (e as CustomEvent).detail;
        if (g.isTransforming && typeof transformUI !== 'undefined' && detail.tool !== Tool.MoveContent.id) {
            console.log("[DEBUG] Auto-committing selection transform due to tool change");
            transformUI.commit();
        }
        // Clear Polygon/Lasso state when switching tools
        if (detail.tool !== Tool.PolySelect.id) {
            window.polyPoints = [];
        }
        if (detail.tool !== Tool.Lasso.id) {
            window.lassoPoints = [];
        }
    });
...
if (typeof window !== 'undefined') {
  Object.assign(window, {
    buildRectSelection, buildEllipseSelection, buildLassoSelection,
    buildPolygonalSelection, buildSelectionFromMask, drawSelectionBorder,
    getRectBorder, getEllipseBorder, selectAll, deselect, invertSelection,
    isPointInSelection, getMaskBounds, moveSelectionMask, moveSelectionContent,
    finalizeMoveContent, magicWandSelection
  });
}
