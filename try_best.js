...
while (offset < buf.length) {
    ...
    // skip 1 byte (LZF flag)
    const dLen = decompressLZF(payload.subarray(1), tileData);

    // Krita 5.x uses a header. In our case, 18126 - 16384 = 1742.
    // Wait, let's try to skip 1742 bytes.
...
canvas.createPNGStream().pipe(fs.createWriteStream('./io-format-tests/krita_try_best.png')).on('finish', () => {
    console.log("Final attempt saved.");
});
