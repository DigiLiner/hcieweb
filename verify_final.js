...
while (offset < buf.length) {
    ...
    let skip = dLen > 16384 ? (dLen - 16384) : 0;
    ...
    for (let row = 0; row < 64; row++) {
        for (let col = 0; col < 64; col++) {
            ...
            if (dx < width && dy < height) {
                ...
                // TRYING RGBA order instead of BGRA
                pixels[docIdx + 0] = tileData[srcIdx + 0 * planeSize]; // R
                pixels[docIdx + 1] = tileData[srcIdx + 1 * planeSize]; // G
                pixels[docIdx + 2] = tileData[srcIdx + 2 * planeSize]; // B
...
canvas.createPNGStream().pipe(fs.createWriteStream('./io-format-tests/krita_solved_by_agent_final_verif.png')).on('finish', () => {
    console.log("Verification image saved.");
});
